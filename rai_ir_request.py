...
def getTrackingDetails(carNumber):
    ...
    session = getSession()
    response_captcha = session.get(url_captcha, verify=False)
    if response_captcha.status_code == 200:
        ...
        response_check = session.post(url_check, data={'txtUserName': login, 'txtPassword': password, 'txtCaptcha': captcha_text, 'B1':'%E6%D1%E6%CF'})
